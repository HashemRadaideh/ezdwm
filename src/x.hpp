...
template <Atom XType>
class XProperty : public XSentinel {
 public:
...
class MutableTextXProperty : public XProperty<XA_TEXT> {
 public:
  ...
  void overwrite(const std::string_view text) const {
    ...
    XChangeProperty(fDisplay, fWindow, fIdentity, utf8Type, 8, PropModeReplace,
...
template <Atom XType>
class MutableXProperty : public XProperty<XType> {
 public:
...
class XNetPropertyFactory {
 public:
  ...
  PropertyType make(const char* name) const {
