  ...
  for (const auto &key : keys) {
    if (const auto code = XKeysymToKeycode(dpy, key.keysym); code) {
      for (const auto &modifier : modifiers) {
        XGrabKey(dpy, code, key.mod | modifier, root, True, GrabModeAsync,
                 GrabModeAsync);
      }
    }
  }
...
Atom getXAtomProperty(Window window, Atom prop) {
  ...
  if (XGetWindowProperty(dpy, window, prop, 0L, sizeof(prop), False, XA_ATOM,
                         ...
                         &bytesAfterReturn, &outProperty) == Success &&
...
int getXTextProperties(Window window, Atom atom, char *text, uint size) {
  ...
  XTextProperty name{};
  if (!XGetTextProperty(dpy, window, &name, atom) || !name.nitems) return 0;
  if (name.encoding == XA_STRING) {
    strncpy(text, (char *)name.value, size - 1);
  } else {
    char **list = nullptr;
    if (int n; XmbTextPropertyToTextList(dpy, &name, &list, &n) >= Success &&
               n > 0 && *list) {
...
Monitor *wintomon(Window w) {
  ...
  for (const auto &monitor : allMonitors) {
    if (w == monitor->fBarID) return monitor.get();
  }
  if (Client *client = wintoclient(w); client) return client->fMonitor;
...
int getMonitorIndex(Monitor *monitor) {
  ...
  for (size_t i = 0; i < allMonitors.size(); i++) {
    if (allMonitors[i].get() == monitor) return i;
  }
...
Monitor *dirtomon(int dir) {
  int newIndex =
      (getMonitorIndex(selmon) + dir + allMonitors.size()) % allMonitors.size();
...
void manageClient(Window window, XWindowAttributes *wa) {
  ...
  if (clientPtr->fMonitor == selmon) unfocus(selmon->fSelected, false);
...
void sendClientToMonitor(Client *client, Monitor *monitor) {
  if (client->fMonitor == monitor) return;
  unfocus(client, true);
...
void drawbars() {
  for (const auto &monitor : allMonitors) monitor->drawbar();
}
...
void updateAllXClientLists() {
  ...
  for (const auto &monitor : allMonitors) monitor->updateXClientList();
}
...
Client::Client(Window win, const Rect &clientRect, int borderWidth)
    : fWindow{win},
      fSize{clientRect},
      fOldSize{clientRect},
      fBorderWidth{borderpx},
      fOldBorderWidth{borderWidth},
      fXName{win, netatom->wmName},
      fXState{win, netatom->wmState} {
  ...
  if (fFlags.isFloating) XRaiseWindow(dpy, fWindow);
...
void Client::resizeXClient(const Rect &newSize) {
  ...
  XConfigureWindow(dpy, fWindow, CWX | CWY | CWWidth | CWHeight | CWBorderWidth,
                   &windowChanges);
  sendXWindowConfiguration();
  XSync(dpy, False);
}
...
void Client::resize(int x, int y, int width, int height, const bool interact) {
  ...
  if (interact) {
    if (x > screenWidth) x = screenWidth - getOuterWidth();
    if (y > screenHeight) y = screenHeight - getOuterHeight();
    if (x + width + 2 * fBorderWidth < 0) x = 0;
    if (y + height + 2 * fBorderWidth < 0) y = 0;
  } else {
  ...
  if (resizehints || fFlags.isFloating ||
    ...
    bool isBaseSizeMin = fBaseWidth == fMinWidth && fBaseHeight == fMinHeight;
    ...
    if (fWidthIncrement) width -= width % fWidthIncrement;
    if (fHeightIncrement) height -= height % fHeightIncrement;

    // Restore base dimensions
    width = std::max(width + fBaseWidth, fMinWidth);
    height = std::max(height + fBaseHeight, fMinHeight);

    if (fMaxWidth) width = std::min(width, fMaxWidth);
    if (fMaxHeight) height = std::min(height, fMaxHeight);
  }
  ...
  do {
    ...
    switch (event.type) {
      ...
      case MapRequest:
        ...
        break;
      case MotionNotify:
        if ((event.xmotion.time - lasttime) <= (1000 / 60)) continue;
        ...
        auto newHeight =
            std::max(event.xmotion.y - originally - 2 * fBorderWidth + 1, 1);
  ...
  do {
    ...
    switch (event.type) {
      ...
      case MotionNotify:
        ...
        int newY = originally + (event.xmotion.y - y);
        ...
        if (std::abs(selmon->wRect.y - newY - selmon->fGapSize) < snap) {
          newY = selmon->wRect.y + selmon->fGapSize;
                   ...
                   snap) {
          newY = selmon->wRect.y + selmon->wRect.height - getOuterHeight() -
                 selmon->fGapSize;
        }
        if (!fFlags.isFloating && selmon->getActiveLayout()->arrange &&
...
void Client::toggleFloating() {
  if (fFlags.isFullscreen) return; /* no support for fullscreen windows */
...
void Client::updatePropertyFromEvent(Atom property) {
  switch (property) {
    case XA_WM_TRANSIENT_FOR:
      if (Window trans; !fFlags.isFloating &&
  ...
  if (property == XA_WM_NAME || property == netatom->wmName) {
    updateWindowTitleFromX();
    if (this == fMonitor->fSelected) fMonitor->drawbar();
  }
  if (property == netatom->wmWindowType) updateWindowTypeFromX();
}
...
void Client::grabXButtons(bool focused) const {
  ...
  for (const auto &button : buttons) {
    if (button.click != ClkClientWin) continue;
    for (const auto &modifier : modifiers) {
      XGrabButton(dpy, button.button, button.mask | modifier, fWindow, False,
...
void Client::handleConfigurationRequest(XConfigureRequestEvent *event) {
  if (event->value_mask & CWBorderWidth) {
    ...
    if ((fSize.x + fSize.width) > fMonitor->sRect.x + fMonitor->sRect.width &&
        fFlags.isFloating) {  // Center x
      fSize.x =
          fMonitor->sRect.x + (fMonitor->sRect.width / 2 - getOuterWidth() / 2);
    }
    if ((fSize.y + fSize.height) > fMonitor->sRect.y + fMonitor->sRect.height &&
...
void Client::applyCustomRules() {
  ...
  std::string_view xclass = classHint.res_class ? classHint.res_class : broken;
  std::string_view instance = classHint.res_name ? classHint.res_name : broken;
...
void Client::updateWindowTypeFromX() {
  ...
  if (state == netatom->wmFullscreen) setFullscreen(true);
  if (wtype == netatom->wmWindowTypeDialog) fFlags.isFloating = true;
}
...
Monitor::Monitor(int num)
    : fGapSize{::gappx},
      fMonitorNumber{num},
      fMasterFactor{::mfact},
      fMasterCount{::nmaster},
      fShouldRenderBar{::showbar},
...
Client *Monitor::getClientFromWindowID(Window win) const {
  ...
  if (client == fClients.end()) return nullptr;
  return client->get();
}
...
void Monitor::setActiveLayout(const Layout *layout) {
  if (layout) fLayouts[fSelectedLayout] = layout;
...
void Monitor::unmanage(Client *ptr, bool xResourceDestroyed) {
  {
    ...
    if (!xResourceDestroyed) client->unmanageAndDestroyX();
  }
  selmon->focus();
...
void Monitor::hideClientsIfInvisible() const {
  for (auto &client : fStack) {
    if (client->isVisible()) client->hideXClientIfInvisible();
  }
...
void Monitor::focus(Client *client) {
  ...
  if (!client || !client->isVisible()) {
    auto loc = std::ranges::find_if(
        fStack, [](const auto &client) { return client->isVisible(); });
    if (loc != fStack.end()) client = *loc;
  }
  if (fSelected && fSelected != client) unfocus(fSelected, false);

  if (client) {
    if (client->getFlags().isUrgent) client->setUrgent(false);
...
void Monitor::shiftFocusThroughStack(int direction) {
  ...
  if (direction > 0) {
    ...
    for (auto &client : fClients) {
      if (fSelected == client.get() && c) break;
      if (client->isVisible()) c = client.get();
    }
  }
  if (c) {
    focus(c);
...
void Monitor::zoomClientToMaster(Client *client) {
  if (!getActiveLayout()->arrange || client->getFlags().isFloating) return;

  if (auto tiledClients = getTiledClients();
      tiledClients && client == tiledClients.front().get()) {
    // TODO: this is a complexity regression
    auto nextTiledClient = std::views::drop_while(
        tiledClients,
        [=](const auto &tiledClient) { return tiledClient.get() != client; });
    if (!nextTiledClient) return;
...
void Monitor::restackClients() const {
  ...
  if (getActiveLayout()->arrange) {
    ...
    for (const auto *client : fStack) {
      if (client->getFlags().isFloating || !client->isVisible()) continue;
...
void Monitor::arrangeClients(bool shouldRestack) {
  ...
  strncpy(fLayoutSymbol, getActiveLayout()->symbol, sizeof(fLayoutSymbol));
  if (getActiveLayout()->arrange) getActiveLayout()->arrange(this);

  if (shouldRestack) restackClients();
}
...
void Monitor::drawbar() const {
  ...
  for (const auto &client : fClients) {
    occ |= client->fTags;
    if (client->getFlags().isUrgent) urg |= client->fTags;
  }
  int x = 0;
  for (size_t i = 0; i < tags.size(); i++) {
    ...
    if (occ & 1 << i) {
      drw->renderRect(
          x + boxs, boxs, boxw, boxw,
          isSelectedMonitor() && fSelected && fSelected->fTags & 1 << i,
          urg & 1 << i);
    }
    x += w;
  }
  ...
  if ((w = wRect.width - tw - x) > barHeight) {
    if (fSelected) {
      drw->setScheme(isSelectedMonitor() ? scheme->selected : scheme->normal);
      drw->renderText(x, 0, w, barHeight, lrpad / 2, fSelected->getName(), 0);
...
void Monitor::updateXGeometry() const {
  for (auto &client : fClients) {
    if (client->getFlags().isFullscreen) client->resizeXClient(sRect);
  }
  XMoveResizeWindow(dpy, fBarID, wRect.x, fBarY, wRect.width, barHeight);
}
...
void Monitor::monocle() {
  ...
  for (auto &client : getTiledClients()) {
    client->resize(wRect.x, wRect.y, wRect.width - 2 * client->getBorderWidth(),
...
void Monitor::tile() {
  ...
  for (const auto &c : getTiledClients()) {
    if (i < fMasterCount) {  // Master window
      auto h = (wRect.height - my) / (std::min(n, fMasterCount) - i) - fGapSize;
      ...
      c->resize(wRect.x + mw + fGapSize, wRect.y + ty,
                wRect.width - mw - (2 * c->getBorderWidth()) - 2 * fGapSize,
                h - (2 * c->getBorderWidth()), false);
...
void clientmessage(XEvent *e) {
  ...
  if (cme->message_type == netatom->wmState) {
    if (static_cast<unsigned long>(cme->data.l[1]) == netatom->wmFullscreen ||
        static_cast<unsigned long>(cme->data.l[2]) == netatom->wmFullscreen) {
    ...
    if (c != selmon->fSelected && !c->getFlags().isUrgent) c->setUrgent(true);
  }
}
...
void configurenotify(XEvent *e) {
  ...
  if (ev->window == root) {
    ...
    if (updateDisplayGeometry() || dirty) {
      ...
      updateBarsXWindows();
      for (const auto &monitor : allMonitors) monitor->updateXGeometry();
      selmon->focus();
      arrangeAllMonitors();
    }
  }
...
void expose(XEvent *e) {
  ...
  if (Monitor * m; ev->count == 0 && (m = wintomon(ev->window))) m->drawbar();
}
...
void maprequest(XEvent *e) {
  ...
  if (!XGetWindowAttributes(dpy, ev->window, &wa) || wa.override_redirect)
    return;
  if (!wintoclient(ev->window)) manageClient(ev->window, &wa);
}
...
void killclient() {
  if (selmon->fSelected) selmon->fSelected->requestKill();
}
...
void setmfact(const float factor) {
  if (selmon->getActiveLayout()->arrange) selmon->incrementMasterFactor(factor);
}
...
void view(const uint tag) {
  if ((tag & TAGMASK) == selmon->getActiveTags()) return;
  selmon->toggleSelectedTagSet();
  if (tag & TAGMASK) selmon->setActiveTags(tag & TAGMASK);
  selmon->focus();
  selmon->arrangeClients();
}

void zoom() {
  if (selmon->fSelected) selmon->zoomClientToMaster(selmon->fSelected);
}
...
void setup() {
  ...
  if (drw->createFontSet(fonts).empty()) die("no fonts could be loaded.");
