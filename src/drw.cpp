...
DisplayFont::DisplayFont(Display* display, FcPattern* pattern)
    : fDisplay{display},
      ...
      fPattern{nullptr} {
  if (!fXfont) die("error, cannot load font from pattern:");
...
DisplayFont::DisplayFont(Display* display, const int screen,
  ...
  if (!fXfont) die("cannot load font from name:");
  if (!fPattern) die("error, cannot parse font name to pattern:");
...
std::optional<DisplayFont> DisplayFont::generateDerivedFontWithCodepoint(
  ...
  if (!match) die("Match fail: TODO: figure out what should happen here");
...
void Drw::renderRect(const int x, const int y, const uint w, const uint h,
  ...
  XSetForeground(
      fDisplay, fGC,
      invert ? fScheme->background.pixel : fScheme->foreground.pixel);
...
int Drw::renderText(int x, const int y, uint w, uint h, const uint lpad,
  ...
  if (shouldRender) {
    XSetForeground(
        fDisplay, fGC,
        invert ? fScheme->foreground.pixel : fScheme->background.pixel);
    XFillRectangle(fDisplay, fDrawable, fGC, x, y, w, h);
    xftDrawer =
  ...
  while (!text.empty()) {
    ...
    if (!renderingFont) {
      ...
      auto newFont =
          fFonts[0].generateDerivedFontWithCodepoint(fScreen, utf8Codepoint);
      if (newFont) {
